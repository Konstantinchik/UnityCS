        ...
        private void LoadResolutionOptions()
        {
            ...
            */
            for (int i = 0; i < Screen.resolutions.Length; i++)
            {
                ...
                _resolution_options.Add(new Dropdown.OptionData
                {
                    text = Screen.resolutions[i].width + "x" + Screen.resolutions[i].height + "@" + refreshRate.ToString("F0") + "Hz"
                });

                if (this.m_ResolutionList[i].width == Screen.currentResolution.width &&
                    this.m_ResolutionList[i].height == Screen.currentResolution.height &&
                    Mathf.Approximately((float)this.m_ResolutionList[i].refreshRateRatio.value, (float)Screen.currentResolution.refreshRateRatio.value))
                {
                    _resolution_index = i;
                }
            }
