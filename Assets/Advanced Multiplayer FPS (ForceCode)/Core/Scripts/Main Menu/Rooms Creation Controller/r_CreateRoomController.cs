...
namespace ForceCodeFPS
{
    ...
    public class GameMap
    {
        ...
        public string m_SceneName; // имя сцены для NGO
    }
    #endregion
    ...
    public class r_CreateRoomController : MonoBehaviour
    {
        ...
        /// <summary>
        /// НОВЫЕ МЕТОДЫ GetRandomMapName() и GetRandomGameMode()
        /// </summary>
        /// <returns></returns>
        public string GetRandomMapName()
        {
            int id = Random.Range(0, m_GameMaps.Length);
            return m_GameMaps[id].m_MapName;
        }

        public string GetRandomGameMode()
        {
            int id = Random.Range(0, m_GameModes.Length);
        ...
        private void HandleButtons()
        {
            ...
            m_RoomUI.m_CreateRoomButton.onClick.AddListener(() =>
            {
                CreateRoom();
                PlayClick();
        ...
        {
            if (r_AudioController.instance != null)
        ...
        // 🔥 Главное отличие: запуск игры как хост и загрузка сцены
        private void CreateRoom()
        {
            var selectedMap = m_GameMaps[m_CurrentGameMap];

            // Здесь можно сохранить выбор в Singleton, ScriptableObjects или через GameSettings
            GameSettings.CurrentMapName = selectedMap.m_MapName;
            GameSettings.CurrentGameMode = m_GameModes[m_CurrentGameMode];
            GameSettings.MaxPlayers = m_PlayerLimit[m_CurrentPlayerLimit];

            NetworkManager.Singleton.StartHost();
            SceneManager.LoadScene(selectedMap.m_SceneName);
        }



        /// <summary>
        /// In this section we are applying the room settings and properties. - ОСТАЛОСЬ ОТ PHOTON
        /// </summary>
