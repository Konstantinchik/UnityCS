...
{
    public class r_PlayerController : NetworkBehaviour
    {
        // Добавляем переменную с правами на изменение только у владельца (Owner)
        ...
        private void SetDefaults()
        {
            if (!IsOwner)
            {
                Destroy(this); // отключаем управление у чужих игроков
                return;
            }

            // Привязать камеру, включить управление и т.д.
            Debug.Log("Я — локальный игрок");
