...
{
    public class r_WeaponController : NetworkBehaviour
    {
        ...
        private NetworkVariable<int> m_Ammo = new NetworkVariable<int>();
        private NetworkVariable<int> m_TotalAmmunation = new NetworkVariable<int>();
        private NetworkVariable<r_WeaponState> m_WeaponState = new NetworkVariable<r_WeaponState>();
        private NetworkVariable<r_WeaponAimState> m_WeaponAimState = new NetworkVariable<r_WeaponAimState>();
        #endregion
        ...
        private void OnDisable()
        {
            ...
            this.m_WeaponState.Value = r_WeaponState.IDLE;
            this.m_WeaponAimState.Value = r_WeaponAimState.HIP;
        }
        ...
        {
            m_CanShoot = Time.time > m_FireCooldown &&
                         m_PlayerController.m_MoveState != r_MoveState.SPRINTING &&
                         !m_Animator.GetCurrentAnimatorStateInfo(0).IsName(m_WeaponConfig.m_AnimationSettings.m_EquipAnimName) &&
                         !m_Animator.GetCurrentAnimatorStateInfo(0).IsName(m_WeaponConfig.m_AnimationSettings.m_UnequipAnimName) &&
                         m_Ammo.Value > 0;

            m_CanReload = m_Ammo.Value < m_Ammunation.m_MaxAmmoClip &&
                          m_WeaponState.Value != r_WeaponState.RELOADING &&
                          !m_Animator.GetCurrentAnimatorStateInfo(0).IsName(m_WeaponConfig.m_AnimationSettings.m_EquipAnimName) &&
                          !m_Animator.GetCurrentAnimatorStateInfo(0).IsName(m_WeaponConfig.m_AnimationSettings.m_UnequipAnimName) &&
                          !m_Animator.GetCurrentAnimatorStateInfo(0).IsName(m_WeaponConfig.m_AnimationSettings.m_FireAnimName);

            m_canAim = m_PlayerController.m_MoveState != r_MoveState.SPRINTING && m_CurrentAimTime <= 0;

            if (m_CanShoot)
            {
                switch (m_WeaponConfig.m_FireModeSettings.m_FireMode)
                {
                    case FireMode.SINGLE:
                        if (m_PlayerController.m_InputManager.GetFireClick())
                            SingleFire();
                        break;
                    case FireMode.AUTOMATIC:
                        if (m_PlayerController.m_InputManager.GetFireHold())
                            SingleFire();
                        break;
                    case FireMode.BURST:
                        if (m_PlayerController.m_InputManager.GetFireClick())
            ...
            if (m_CanReload && m_PlayerController.m_InputManager.GetReloadKey())
                OnWeaponReload();

            m_WeaponAimState.Value = m_PlayerController.m_InputManager.GetAimKey() && m_canAim ?
                r_WeaponAimState.AIMING : r_WeaponAimState.HIP;
        }
        ...
        private void HandleWeaponMotion()
        {
            ...
            if (this.m_weaponCrouchTransform != null && this.m_WeaponConfig.m_WeaponMotionSettings.m_CrouchFunction)
            {
                ...
                this.m_weaponCrouchTransform.localPosition = Vector3.Lerp(this.m_weaponCrouchTransform.localPosition, this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? Vector3.zero : _crouch_position, this.m_WeaponConfig.m_WeaponMotionSettings.m_CrouchSpeed * Time.deltaTime);
                this.m_weaponCrouchTransform.localRotation = Quaternion.Lerp(this.m_weaponCrouchTransform.localRotation, this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? Quaternion.identity : Quaternion.Euler(_crouch_rotation), this.m_WeaponConfig.m_WeaponMotionSettings.m_CrouchSpeed * Time.deltaTime);
            }
        ...
        private void HandleWeaponAnimatorMotion()
        {
            ...
            if (this.m_PlayerController.m_MoveState == r_MoveState.IDLE || this.m_WeaponAimState.Value == r_WeaponAimState.AIMING)
            {
                //0 = Idle
                _move_state = 0;
            }
        ...
        private void HandleWeaponAim()
        {
            ...
            this.m_PlayerCamera.m_CameraState = m_WeaponAimState.Value == r_WeaponAimState.AIMING ? r_CameraState.AIMING : r_CameraState.HIP;

            //Set weapon camera data for aim settings
            SetWeaponCameraData();

            //Weapon aim position and rotation
            Vector3 _aimPosition = this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? this.m_WeaponConfig.m_weaponAimSettings.m_aimPosition : this.m_WeaponConfig.m_weaponAimSettings.m_hipPosition;
            Vector3 _aimRotation = this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? this.m_WeaponConfig.m_weaponAimSettings.m_aimRotation : this.m_WeaponConfig.m_weaponAimSettings.m_hipRotation;
        ...
        private void HandleWeaponSway()
        {
            ...
            float _swayMultiplier = this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? this.m_WeaponConfig.m_weaponSwaySettings.m_swayMultiplerAim : this.m_WeaponConfig.m_weaponSwaySettings.m_swayMultiplier;
        ...
        {
            if (m_WeaponConfig.m_ReloadSettings.m_reloadType == r_ReloadType.SINGLE && m_BulletByBulletReloadCorountine != null)
            {
                StopCoroutine(m_BulletByBulletReloadCorountine);
                m_BulletByBulletReloadCorountine = null;
            }

            m_WeaponState.Value = r_WeaponState.FIRING;
            PlayAnimation(r_AnimationType.PLAY, m_WeaponConfig.m_AnimationSettings.m_FireAnimName, 0, false);

            // Требуется перевести ThirdPersonManager на NGO
            m_PlayerController.m_ThirdPersonManager.OnWeaponFire();
            m_PlayerController.m_PlayerUI.OnCrosshairShoot(m_WeaponConfig.m_weaponCrosshairSettings.m_CrosshairIncreaseSize);

            SetRecoil();
            if (m_WeaponConfig.m_WeaponKickbackSettings.m_KickbackFunction)
                SetKickback();

            PlaySound(m_WeaponConfig.m_WeaponSound.m_FireSoundName, m_WeaponConfig.m_WeaponSound.m_FireSoundVolume, true);
            SetMuzzleflash();
            StartCoroutine(SetShellEject());

            m_Ammo.Value--;

            if (m_WeaponConfig.m_ReloadSettings.m_AutoReload && m_Ammo.Value == 0 && m_TotalAmmunation.Value > 0)
                OnWeaponReload();

            m_FireCooldown = Time.time + m_WeaponConfig.m_FireModeSettings.m_FireRate;

            for (int i = 0; i < m_WeaponConfig.m_BulletSettings.m_ShotFragments; i++)
            {
                switch (m_WeaponConfig.m_BulletSettings.m_BulletType)
                {
        ...
        {
            if (Physics.Raycast(m_PlayerCamera.m_PlayerCamera.transform.position, GetSpread() * Vector3.forward,
                out RaycastHit _hit, m_WeaponConfig.m_BulletSettings.m_ShootDistance, m_LayerMask.value))
            {
                ...
                {
                    if (m_PlayerController.m_PlayerAudio.GetBulletImpact(_hit.collider, _hit.point) != null)
                    {
                        GameObject _bulletImpact = Instantiate(
                            Resources.Load<GameObject>("Impacts/" + m_PlayerController.m_PlayerAudio.GetBulletImpact(_hit.collider, _hit.point).name),
                            _hit.point + (_hit.normal * 0.01f),
                            Quaternion.LookRotation(_hit.normal));

                        _bulletImpact.transform.parent = _hit.collider.transform.root.parent;
                        StartCoroutine(DestroyObject(_bulletImpact, 2f));
                    }

                    if (_hit.transform.GetComponent<Rigidbody>() != null)
                    {
                        _hit.transform.GetComponent<Rigidbody>().AddForceAtPosition(
                            transform.forward * m_WeaponConfig.m_BulletSettings.m_bulletImpactForce,
                            _hit.point);
                    }
                }
            }
        ...
        {
            if (_enemy.m_PlayerHealth != null && !_enemy.m_PlayerHealth.m_IsDeath)
            {
                if (_enemy.m_PlayerHealth.m_Health.Value > 0)
                {
                    // Требуется перевести PlayerHealth на NGO
                    _enemy.m_PlayerHealth.DecreaseHealth(
                        NetworkManager.Singleton.LocalClient.PlayerObject.GetComponent<r_PlayerNetwork>().PlayerName.Value,
                        GetBodyPartWeaponDamage(_enemy.m_BodyParts.m_BodyPartType),
                        m_PlayerController.transform.position,
                        m_PlayerController.m_PlayerHealth.m_Health,
                        m_WeaponConfig.m_WeaponInformationSettings.m_WeaponName);
                }

                if (_enemy.m_PlayerHealth.m_Health.Value <= 0f)
                {
                    PlaySound(m_WeaponConfig.m_WeaponSound.m_EnemyKilledSoundName,
                        m_WeaponConfig.m_WeaponSound.m_EnemyKilledVolume, false);

                    m_PlayerController.m_PlayerUI.SetHitmarker(true);
                    m_PlayerController.m_PlayerUI.SetKillmessage(
                        _enemy.m_PlayerHealth.m_PlayerController.m_PlayerNetwork.PlayerName.Value);

                    // Требуется перевести KillfeedManager на NGO
                    r_KillfeedManager.Instance.AddKillfeed(
                        NetworkManager.Singleton.LocalClient.PlayerObject.GetComponent<r_PlayerNetwork>(),
                        m_WeaponConfig.m_WeaponInformationSettings.m_WeaponName,
                        _enemy.m_PlayerHealth.m_PlayerController.m_PlayerNetwork);
                }
                else
                {
                    m_PlayerController.m_PlayerUI.SetHitmarker(false);
                }
            }

            _enemy.m_BodyParts.m_BodyPartRigidbody.AddForceAtPosition(
                -_hit.transform.forward * m_WeaponConfig.m_BulletSettings.m_ragdollImpactForce,
                _hit.point);

            if (m_WeaponConfig.m_weaponDamageEffectSettings.m_bodyImpact != null)
            {
                GameObject _damageEffect = Instantiate(
                    Resources.Load<GameObject>("Impacts/" + m_WeaponConfig.m_weaponDamageEffectSettings.m_bodyImpact.name),
                    _hit.point,
                    Quaternion.LookRotation(_hit.normal));

                StartCoroutine(DestroyObject(_damageEffect, 4f));
            }

            PlaySound(m_WeaponConfig.m_WeaponSound.m_BodyImpactSoundName,
                m_WeaponConfig.m_WeaponSound.m_BodyImpactVolume, false);
        }
        ...
        {
            //Set Weapon State
            this.m_WeaponState.Value = r_WeaponState.RELOADING;
        ...
        private IEnumerator SetShellEject()
        {
            ...
            Vector3 _shellRotation = new Vector3(UnityEngine.Random.Range(-this.m_WeaponConfig.m_weaponFXSettings.m_shellRandomRotation.x, this.m_WeaponConfig.m_weaponFXSettings.m_shellRandomRotation.x), UnityEngine.Random.Range(-this.m_WeaponConfig.m_weaponFXSettings.m_shellRandomRotation.y, this.m_WeaponConfig.m_weaponFXSettings.m_shellRandomRotation.y), UnityEngine.Random.Range(-this.m_WeaponConfig.m_weaponFXSettings.m_shellRandomRotation.z, this.m_WeaponConfig.m_weaponFXSettings.m_shellRandomRotation.z));
        ...
        {
            //Add recoil
            this.m_targetRecoil += new Vector3(-this.m_WeaponConfig.m_weaponRecoilSettings.m_recoilEuler.x, UnityEngine.Random.Range(-this.m_WeaponConfig.m_weaponRecoilSettings.m_recoilEuler.y, this.m_WeaponConfig.m_weaponRecoilSettings.m_recoilEuler.y), UnityEngine.Random.Range(-this.m_WeaponConfig.m_weaponRecoilSettings.m_recoilEuler.z, this.m_WeaponConfig.m_weaponRecoilSettings.m_recoilEuler.y));
        }
        ...
        private void SetKickback()
        {
            ...
            this.m_WeaponKickbackTransform.localPosition = this.m_WeaponKickbackTransform.localPosition + (this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? (this.m_WeaponConfig.m_WeaponKickbackSettings.m_KickBackPositionAmount * this.m_WeaponConfig.m_WeaponKickbackSettings.m_KickBackAimStabilizationMultiplier) : this.m_WeaponConfig.m_WeaponKickbackSettings.m_KickBackPositionAmount);
            this.m_WeaponKickbackTransform.localEulerAngles = this.m_WeaponKickbackTransform.localEulerAngles + (this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? (this.m_WeaponConfig.m_WeaponKickbackSettings.m_KickBackRotationAmount * this.m_WeaponConfig.m_WeaponKickbackSettings.m_KickBackAimStabilizationMultiplier) : this.m_WeaponConfig.m_WeaponKickbackSettings.m_KickBackRotationAmount);
        }
        ...
        private void PlaySound(string audioClipName, float audioVolume, bool networked)
        {
            // Только локальный игрок вызывает звук
            if (IsOwner)
            {
                m_PlayerAudio.OnWeaponAudioPlay(m_WeaponConfig.m_WeaponInformationSettings.m_WeaponName, audioClipName, audioVolume, networked);

                // Если нужно проиграть звук по сети — вызываем RPC
                if (networked)
                {
                    PlaySoundClientRpc(audioClipName, audioVolume);
                }
            }
        }

        [ClientRpc]
        private void PlaySoundClientRpc(string audioClipName, float audioVolume)
        {
            // Все клиенты, кроме владельца (он уже проиграл звук локально)
            if (!IsOwner)
            {
                m_PlayerAudio.OnWeaponAudioPlay(m_WeaponConfig.m_WeaponInformationSettings.m_WeaponName, audioClipName, audioVolume, false);
            }
        }
        ...
        private Quaternion GetSpread()
        {
            ...
            float _MinSpread = this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? -this.m_WeaponConfig.m_BulletSettings.m_AimSpread : -this.m_WeaponConfig.m_BulletSettings.m_HipSpread;
            float _MaxSpread = this.m_WeaponAimState.Value == r_WeaponAimState.AIMING ? this.m_WeaponConfig.m_BulletSettings.m_AimSpread : this.m_WeaponConfig.m_BulletSettings.m_HipSpread;

            //Apply and return the min/max spread
            return Quaternion.RotateTowards(Quaternion.LookRotation(this.m_PlayerCamera.m_PlayerCamera.transform.forward), UnityEngine.Random.rotation, UnityEngine.Random.Range(_MinSpread, _MaxSpread));
        }
